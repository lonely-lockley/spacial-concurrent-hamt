...
import com.github.lonelylockley.spacial.ctrie.Gen;
...
public class NodeWrapper<T, V> extends BaseNode<T, V> {
    ...
    public final BaseNode<T, V> getGCAS(SpacialConcurrentTrieMap<T, V> instance) {
    ...
    private BaseNode<T, V> getGCASInternal(BaseNode<T, V> copy, final SpacialConcurrentTrieMap<T, V> instance) {
        while (true) {
            if (copy == null) {
                return null;
            }
    ...
    protected final boolean setGCAS(final BaseNode<T, V> oldValue, final BaseNode<T, V> newValue, final SpacialConcurrentTrieMap<T, V> ct) {
    ...
    @Override
    public final int cachedSize(final SpacialConcurrentTrieMap<T, V> instance) {
    ...
    public final NodeWrapper<T, V> copyToGen(final Gen newGeneration, final SpacialConcurrentTrieMap<T, V> instance) {
    ...
    private void clean(final NodeWrapper<T, V> nd, final SpacialConcurrentTrieMap<T, V> ct, int offset) {
    ...
    public final boolean insertInternal(final H3CellId<T> key, final V value, final long hash, final int offset, final NodeWrapper<T, V> parent, final Gen startgen, final SpacialConcurrentTrieMap<T, V> instance) {
        while(true) {
     ...
     */
    public final Optional<V> insertWithConditionInternal(final H3CellId<T> key, final V value, final V cond, final int offset, final NodeWrapper<T, V> parent, final Gen startgen, final SpacialConcurrentTrieMap<T, V> instance) {
        while (true) {
            BaseNode<T, V> m = getGCAS(instance); // use -Yinline!
            if (m instanceof BranchNode<T, V> cn) {
                // 1) a multiway node
    ...
    private boolean insertIntoCollisionAwareNode(final CollisionAwareNode<T, V> ln, final H3CellId key, final V value, final SpacialConcurrentTrieMap<T, V> ct) {
        CollisionAwareNode<T, V> nn = ln.inserted(key, value);
        return setGCAS(ln, nn, ct);
    }

    private final void cleanParent(final BaseNode<T, V> nonlive, final NodeWrapper<T, V> parent, final SpacialConcurrentTrieMap<T, V> instance, final long hash, final int offset, final Gen startgen) {
        while (true) {
     ...
     *         value otherwise
     */
    public final Optional<V> removeInternal(H3CellId<T> key, V value, int offset, final NodeWrapper<T, V> parent, final Gen startgen, final SpacialConcurrentTrieMap<T, V> instance) {
        BaseNode<T, V> m = getGCAS(instance); // use -Yinline!
        if (m instanceof BranchNode<T, V> cn) {
            final int idx;
            final int pos;
     ...
     */
    public final Object lookupInternal(final H3CellId<T> key, int offset, NodeWrapper<T, V> parent, final Gen startgen, final SpacialConcurrentTrieMap<T, V> instance) {
        while (true) {
            BaseNode<T, V> m = getGCAS(instance); // use -Yinline!
            if (m instanceof BranchNode<T, V> cn) {
                // 1) a multinode
     ...
     *         wasn't successful, or any other value otherwise
     */
    public Object subTreeInternal(final H3CellId<T> key, int offset, final int resolution, NodeWrapper<T, V> parent, final Gen startgen, final SpacialConcurrentTrieMap<T, V> instance) {
        while (true) {
            BaseNode<T, V> m = getGCAS(instance); // use -Yinline!
            if (m instanceof BranchNode<T, V> cn) {
                // 1) a multinode
    ...
    private Object cleanReadOnly(final TombstoneNode<T, V> tn, final int offset, final NodeWrapper<T, V> parent, final SpacialConcurrentTrieMap<T, V> ct, H3CellId<T> k, long hc) {
        if (!ct.isReadOnly()) {
