...
public class SpacialConcurrentTrieMap<T, V> extends AbstractMap<H3CellId<T>, V> implements ConcurrentMap<H3CellId<T>, V>, Serializable {

    private static final AtomicReferenceFieldUpdater<SpacialConcurrentTrieMap, BaseNode> ROOT_UPDATER = AtomicReferenceFieldUpdater.newUpdater(SpacialConcurrentTrieMap.class, BaseNode.class, "root");
    ...
    static {
        final Field f;
        try {
            f = SpacialConcurrentTrieMap.class.getDeclaredField("readOnly");
        }
    ...
    protected SpacialConcurrentTrieMap(final NodeWrapper<T, V> r, boolean readOnly) {
        this.readOnly = readOnly;
        this.root = r;
    }

    public SpacialConcurrentTrieMap() {
    ...
    public final SpacialConcurrentTrieMap<T, V> subTree(final H3CellId<T> key) {
        while (true) {
            NodeWrapper<T, V> r = getRootRDCSS();
            Object res = r.subTreeInternal(key, H3CellId.BASE_OFFSET, 0, null, r.gen, this);
            if (res != NodeWrapper.RESTART) {
                res = res == null ? NodeWrapper.newRootNode() : res;
                return new SpacialConcurrentTrieMap<>((NodeWrapper<T, V>) res, true);
            }
        }
    }
    ...
    public final SpacialConcurrentTrieMap<T, V> snapshot() {
        while (true) {
            NodeWrapper<T, V> r = getRootRDCSS();
            final BaseNode<T, V> expmain = r.getGCAS(this);
            if (swapRootRDCSS(r, expmain, r.copyToGen(new Gen(), this))) {
                return new SpacialConcurrentTrieMap<>(r.copyToGen(new Gen(), this), readOnly);
            }
        }
    }
     ...
     */
    public final SpacialConcurrentTrieMap<T, V> readOnlySnapshot() {
        ...
        while (true) {
            ...
            if (swapRootRDCSS(r, expmain, r.copyToGen(new Gen(), this))) {
                return new SpacialConcurrentTrieMap<>(r, true);
            }
        }
    }
    ...
    protected SpacialConcurrentTrieMap<T, V> add(H3CellId<T> key, V value) {
        update(key, value);
        return this;
    }
    ...
    final class EntrySet extends AbstractSet<Entry<H3CellId<T>, V>> implements Serializable {
        ...
        @Override
        public Iterator<Entry<H3CellId<T>, V>> iterator() {
            return SpacialConcurrentTrieMap.this.iterator();
        }
        ...
        public boolean remove(final Object o) {
            ...
            final Entry<H3CellId<T>, V> e = (Entry<H3CellId<T>, V>) o;
            final H3CellId<T> k = e.getKey();
            return null != SpacialConcurrentTrieMap.this.remove(k);
        }
        ...
        @Override
        public void clear() {
            SpacialConcurrentTrieMap.this.clear();
        }
    }
